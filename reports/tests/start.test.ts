...
import { ReportsBackendApp } from '../src/reports-backend-app';

jest.mock('../src/ReportsBackendApp');
...
describe('start.ts', () => {
  ...
  it('should start the ReportsBackendApp', () => {
    require('../src/start');
    expect(ReportsBackendApp.prototype.start).toHaveBeenCalled();
  });
  it('should handle errors during app start', () => {
    const exitMock = mockProcessExit(1);
    (ReportsBackendApp.prototype.start as jest.Mock).mockImplementationOnce(() => {
