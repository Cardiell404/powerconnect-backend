...
import { BillingBackendApp } from '../src/billing-backend-app';

jest.mock('../src/BillingBackendApp');
...
describe('start.ts', () => {
  ...
  it('should start the BillingBackendApp', () => {
    require('../src/start');
    expect(BillingBackendApp.prototype.start).toHaveBeenCalled();
  });
  it('should handle errors during app start', () => {
    const exitMock = mockProcessExit(1);
    (BillingBackendApp.prototype.start as jest.Mock).mockImplementationOnce(() => {
