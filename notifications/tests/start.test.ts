...
import { NotificationsBackendApp } from '../src/notifications-backend-app';

jest.mock('../src/NotificationsBackendApp');
...
describe('start.ts', () => {
  ...
  it('should start the NotificationsBackendApp', () => {
    require('../src/start');
    expect(NotificationsBackendApp.prototype.start).toHaveBeenCalled();
  });
  it('should handle errors during app start', () => {
    const exitMock = mockProcessExit(1);
    (NotificationsBackendApp.prototype.start as jest.Mock).mockImplementationOnce(() => {
