...
import { ClientsBackendApp } from '../src/clients-backend-app';

jest.mock('../src/ClientsBackendApp');
...
describe('start.ts', () => {
  ...
  it('should start the ClientsBackendApp', () => {
    require('../src/start');
    expect(ClientsBackendApp.prototype.start).toHaveBeenCalled();
  });
  it('should handle errors during app start', () => {
    const exitMock = mockProcessExit(1);
    (ClientsBackendApp.prototype.start as jest.Mock).mockImplementationOnce(() => {
