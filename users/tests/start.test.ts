...
import { UsersBackendApp } from '../src/users-backend-app';

jest.mock('../src/UsersBackendApp');
...
describe('start.ts', () => {
  ...
  it('should start the UsersBackendApp', () => {
    require('../src/start');
    expect(UsersBackendApp.prototype.start).toHaveBeenCalled();
  });
  it('should handle errors during app start', () => {
    const exitMock = mockProcessExit(1);
    (UsersBackendApp.prototype.start as jest.Mock).mockImplementationOnce(() => {
